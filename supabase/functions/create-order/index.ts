...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (managerOverride && !['admin', 'manager'].includes(userRole)) {
      return errorResponse(
        'FORBIDDEN',
        'Chỉ Manager/Admin mới được override.',
        null,
        403,
      );
    }
    ...
    const { data: customer, error: custErr } = await supabaseAdmin
      .from('customers')
      .select(
        'id, name, credit_limit, current_debt, overdue_debt, credit_status',
      )
      .eq('id', customerId)
      .single();
    ...
    if (
      ...
      Number(customer.current_debt) + orderTotal >
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.widthCm) {
        rollQuery = rollQuery
          .gte('width_cm', item.widthCm - 1)
          .lte('width_cm', item.widthCm + 1);
      }
      ...
      if (rollErr) {
        return errorResponse(
          'DB_ERROR',
          `Lỗi truy vấn tồn kho: ${rollErr.message}`,
          null,
          500,
        );
      }

      const availableRolls = (rolls ?? []).filter(
        (r) => Number(r.length_m) > 0,
      );
      const totalAvailable = availableRolls.reduce(
        (sum, r) => sum + Number(r.length_m),
        0,
      );
    ...
    const { data: result, error: txErr } = await supabaseAdmin.rpc(
