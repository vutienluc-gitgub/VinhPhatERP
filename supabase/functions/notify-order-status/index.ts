...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { orderId, newStatus, message }: OrderStatusPayload =
      await req.json();
    ...
    const notificationPayload = {
      ...
      message:
        message ??
        `Đơn hàng ${order.order_number} đã được cập nhật sang trạng thái: ${newStatus}`,
    };

    console.log(
      '[notify-order-status] Notification payload:',
      notificationPayload,
    );
