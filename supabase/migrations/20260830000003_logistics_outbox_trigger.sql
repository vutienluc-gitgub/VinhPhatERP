-- ============================================================
-- Migration: Logistics Outbox Dispatcher Trigger (LES v2.1)
-- Description: Projects Domain Events into Shipment Chat Rooms
-- and dispatches Web Push notifications via Edge Function.
-- ============================================================
...
BEGIN
  ...
  IF NEW.payload->>'actor_id' IS NOT NULL THEN
  ...
  -- 5. Insert Projection into Chat Messages
  IF v_room_id IS NOT NULL AND v_msg_content IS NOT NULL THEN
    INSERT INTO public.chat_messages (
      tenant_id, room_id, sender_id, message_type, content, status, created_at
    ) VALUES (
      NEW.tenant_id, v_room_id, NULL, v_msg_type, v_msg_content, 'sent', NOW()
    );
  END IF;
...
DROP TRIGGER IF EXISTS trg_logistics_outbox_dispatched ON public.logistics_outbox_events;
CREATE TRIGGER trg_logistics_outbox_dispatched
  BEFORE INSERT ON public.logistics_outbox_events
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_fn_logistics_outbox_dispatched();
