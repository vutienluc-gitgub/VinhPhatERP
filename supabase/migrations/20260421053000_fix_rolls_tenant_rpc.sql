-- Fix RPCs to include tenant_id when inserting weaving_invoice_rolls

-- Correct original RPCs
CREATE OR REPLACE FUNCTION rpc_create_weaving_invoice(
  p_header JSONB,
  p_rolls JSONB
) RETURNS JSONB AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION rpc_update_weaving_invoice(
  p_id UUID,
  p_header JSONB,
  p_rolls JSONB
) RETURNS VOID AS $$
...
BEGIN
  ...
  -- Get tenant_id from existing invoice to assure security
  SELECT tenant_id INTO v_tenant_id FROM weaving_invoices WHERE id = p_id;
