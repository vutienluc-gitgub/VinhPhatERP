-- ============================================================
-- Migration: Logistics Execution Subsystem RPCs (LES v2.1)
-- Description: Atomic RPCs with Idempotency, Concurrency Guard (OCC),
-- ePOD Evidence Persistence & Transactional Outbox.
-- ============================================================

-- ── 1. RPC: Transition Delivery Attempt State ──
CREATE OR REPLACE FUNCTION public.rpc_transition_delivery_attempt(
  p_command_id UUID,
  p_attempt_id UUID,
...
BEGIN
  ...
  -- Fallback tenant resolution from aggregate if caller is service-role or background worker
  ...
  -- 2. Concurrency Guard (OCC) & State Transition
  UPDATE public.delivery_attempts
  SET
    state = p_target_state,
    notes = COALESCE(p_notes, notes),
    started_at = CASE
      WHEN p_target_state = 'in_transit' AND started_at IS NULL THEN NOW()
      ELSE started_at
    END,
    completed_at = CASE
      WHEN p_target_state IN ('delivered', 'completed', 'returned', 'cancelled') THEN NOW()
      ELSE completed_at
    END,
    updated_at = NOW()
  WHERE id = p_attempt_id
  ...
  IF v_attempt.id IS NULL THEN
    RAISE EXCEPTION 'CONCURRENCY_CONFLICT: Trạng thái hiện tại của chuyến giao không khớp (expected=%)', p_expected_state;
  END IF;

  -- 3. Determine Event Type
  v_event_type := CASE p_target_state
    WHEN 'pending_pickup' THEN 'DELIVERY.ATTEMPT_ASSIGNED'
    WHEN 'picked_up' THEN 'DELIVERY.PICKUP_CONFIRMED'
    WHEN 'in_transit' THEN 'DELIVERY.TRANSIT_STARTED'
    WHEN 'arrived' THEN 'DELIVERY.DRIVER_ARRIVED'
    WHEN 'delivered' THEN 'DELIVERY.EPOD_SUBMITTED'
    WHEN 'completed' THEN 'DELIVERY.ATTEMPT_COMPLETED'
    WHEN 'cancelled' THEN 'DELIVERY.ATTEMPT_CANCELLED'
    ELSE 'DELIVERY.STATE_CHANGED'
  END;
  ...
  INSERT INTO public.logistics_outbox_events (
    ...
    v_tenant_id, v_event_type, 'delivery_attempt', p_attempt_id,
  ...
  -- 5. Prepare and Save Idempotency Result
  v_result := jsonb_build_object(
    'ok', true,
    'attempt_id', v_attempt.id,
    'stop_id', v_attempt.stop_id,
    'previous_state', p_expected_state,
    'current_state', v_attempt.state,
  ...
  INSERT INTO public.logistics_command_idempotency (
    ...
    p_command_id, v_tenant_id, 'transition_delivery_attempt', p_attempt_id, v_user_id, v_result
  );
...
GRANT EXECUTE ON FUNCTION public.rpc_transition_delivery_attempt(UUID, UUID, VARCHAR, VARCHAR, JSONB, TEXT) TO authenticated;


-- ── 2. RPC: Submit Delivery ePOD Evidence ──
...
BEGIN
  ...
  INSERT INTO public.shipment_epod_evidences (
    ...
    COALESCE(v_user_id, '00000000-0000-0000-0000-000000000000'::UUID), NOW(),
...
-- ── 3. RPC: Report Delivery Exception ──
CREATE OR REPLACE FUNCTION public.rpc_report_delivery_exception(
  p_command_id UUID,
  p_attempt_id UUID,
...
BEGIN
  ...
  -- 2. Update Attempt State to 'failed_attempt'
  UPDATE public.delivery_attempts
  SET
  ...
  IF v_attempt.id IS NULL THEN
    RAISE EXCEPTION 'Không tìm thấy chuyến giao hàng (id=%)', p_attempt_id;
  END IF;

  -- 3. Insert Delivery Exception
  INSERT INTO public.delivery_exceptions (
    tenant_id, attempt_id, exception_type, reason_detail,
    status, reported_by, reported_at, resolution_action, resolution_notes
  ) VALUES (
    v_tenant_id, p_attempt_id, p_exception_type, p_reason_detail,
    'OPEN', COALESCE(v_user_id, '00000000-0000-0000-0000-000000000000'::UUID), NOW(),
    p_resolution_action, p_resolution_notes
  )
  ...
  INSERT INTO public.logistics_outbox_events (
    ...
    v_tenant_id, 'DELIVERY.EXCEPTION_OCCURRED', 'delivery_attempt', p_attempt_id,
    v_attempt.correlation_id, p_command_id,
    jsonb_build_object(
      'attempt_id', p_attempt_id,
      'stop_id', v_attempt.stop_id,
  ...
  INSERT INTO public.logistics_command_idempotency (
    ...
    p_command_id, v_tenant_id, 'report_delivery_exception', p_attempt_id, v_user_id, v_result
  );
...
GRANT EXECUTE ON FUNCTION public.rpc_report_delivery_exception(UUID, UUID, VARCHAR, TEXT, VARCHAR, TEXT, JSONB) TO authenticated;
