-- Create ENUM type for stock display
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'public_stock_display_type') THEN
    CREATE TYPE public_stock_display_type AS ENUM ('none', 'status', 'quantity');
  END IF;
END$$;

-- Add new columns to fabric_commercials
ALTER TABLE public.fabric_commercials
ADD COLUMN IF NOT EXISTS public_stock_display public_stock_display_type DEFAULT 'status',
ADD COLUMN IF NOT EXISTS trust_has_sample BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS trust_fast_delivery BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS trust_tech_support BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS standard_consumption_kg NUMERIC DEFAULT 0.25
  CHECK (standard_consumption_kg >= 0.05 AND standard_consumption_kg <= 2.0);

-- Update rpc_get_public_fabric_basic
...
-- Update rpc_get_public_fabric_variants
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      ELSE NULL END,
      'stock_status', CASE 
        WHEN v_stock_display = 'none' AND auth.uid() IS NULL THEN NULL
        WHEN COALESCE(s.available_kg, 0) > 0 THEN 'in-stock' 
        ELSE 'out-of-stock' 
      END,
      'available_kg', CASE 
        WHEN auth.uid() IS NOT NULL OR v_stock_display = 'quantity' THEN COALESCE(s.available_kg, 0) 
        ELSE NULL 
      END,
      'roll_count', CASE 
        WHEN auth.uid() IS NOT NULL OR v_stock_display = 'quantity' THEN COALESCE(s.roll_count, 0) 
        ELSE NULL 
      END
