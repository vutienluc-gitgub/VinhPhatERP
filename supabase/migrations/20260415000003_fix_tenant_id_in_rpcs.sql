-- =============================================================================
-- Migration: Fix tenant_id missing in all atomic_create_* RPCs
-- SECURITY DEFINER bypasses RLS, so tenant_id is never auto-set.
-- Records were inserted with NULL tenant_id -> invisible to RLS queries.
-- =============================================================================
...
BEGIN
  ...
  IF p_items IS NOT NULL AND jsonb_array_length(p_items) > 0 THEN
    INSERT INTO yarn_receipt_items (
      ...
      lot_number, tensile_strength, composition, origin, yarn_catalog_id, sort_order, tenant_id
    )
    SELECT
      v_receipt_id,
      item->>'yarn_type',
...
BEGIN
  ...
  INSERT INTO quotations (
    ...
    delivery_terms, payment_terms, notes, status, tenant_id
  ) VALUES (
    v_quotation_number,
    (p_header->>'customer_id')::UUID,
  ...
  IF p_items IS NOT NULL AND jsonb_array_length(p_items) > 0 THEN
    INSERT INTO quotation_items (
      ...
      width_cm, unit, quantity, unit_price, lead_time_days, notes, sort_order, tenant_id
    )
    SELECT
      v_quotation_id,
      item->>'fabric_type',
...
BEGIN
  ...
  INSERT INTO weaving_invoices (
    ...
    unit_price_per_kg, notes, status, created_by, tenant_id
  ) VALUES (
    v_invoice_number,
    (p_header->>'supplier_id')::UUID,
  ...
  IF p_rolls IS NOT NULL AND jsonb_array_length(p_rolls) > 0 THEN
    INSERT INTO weaving_invoice_rolls (
      ...
      quality_grade, warehouse_location, lot_number, notes, sort_order, tenant_id
    )
    SELECT
      v_invoice_id,
      r->>'roll_number',
...
BEGIN
  ...
  INSERT INTO payments (
    ...
    amount, payment_method, account_id, reference_number, tenant_id
  ) VALUES (
    v_payment_number,
  ...
  FROM payments t
  WHERE t.payment_number = v_payment_number AND t.tenant_id = v_tenant;
...
BEGIN
  ...
  SELECT to_jsonb(t) INTO v_result
  FROM expenses t
  WHERE t.expense_number = v_expense_number AND t.tenant_id = v_tenant;
...
-- ─── 9. Fix orphaned records (tenant_id IS NULL) ──────────────────────────────
-- Update any records that were created without tenant_id
DO $$
DECLARE
...
BEGIN
  ...
  IF v_default_tenant IS NOT NULL THEN
    UPDATE yarn_receipts SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE yarn_receipt_items SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE orders SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE order_items SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE quotations SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE quotation_items SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE shipments SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE shipment_items SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE dyeing_orders SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE dyeing_order_items SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE weaving_invoices SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
    UPDATE weaving_invoice_rolls SET tenant_id = v_default_tenant WHERE tenant_id IS NULL;
