-- ============================================================
-- Migration: Optimize Chat Performance & Unread Synchronization
-- Target: Fix unread_count overhead & introduce atomic mark-as-read RPC
-- Date: 2026-09-16
-- ============================================================

-- 1. Covering Composite Index for LATERAL last_message lookup & cursor pagination
CREATE INDEX IF NOT EXISTS idx_chat_messages_room_deleted_created
ON public.chat_messages(room_id, created_at DESC)
WHERE deleted_at IS NULL;

-- 2. Atomic RPC: Mark Chat Room as Read
-- Resets unread_count to 0 and updates last_read_at / last_read_message_id atomically
...
BEGIN
  ...
  SET
    unread_count = 0,
    last_read_at = NOW(),
    last_read_message_id = COALESCE(p_last_read_message_id, last_read_message_id)
  WHERE room_id = p_room_id
    AND user_id = v_user_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.rpc_mark_chat_room_read(UUID, UUID) TO authenticated;

-- 3. Optimized RPC: rpc_get_my_chat_rooms_v2
-- Replaces expensive dynamic COUNT(*) subquery with O(1) p.unread_count column
...
BEGIN
  ...
  SELECT
    ...
    r.updated_at,
    p.unread_count::BIGINT AS unread_count,
