-- Migration: Fix missing tenant_id in yarn_receipt_items when creating/updating via RPC
-- 20260508135809_add_yarn_receipt_landed_cost.sql accidentally dropped tenant_id from items insert
...
-- Fix orphaned items
UPDATE yarn_receipt_items
SET tenant_id = yarn_receipts.tenant_id
FROM yarn_receipts
WHERE yarn_receipt_items.receipt_id = yarn_receipts.id
  AND yarn_receipt_items.tenant_id IS NULL;
