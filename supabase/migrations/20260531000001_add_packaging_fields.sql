-- Migration: Add packaging specification fields to yarn_receipt_items
-- Fields from carton labels: cones_per_box, box_count, box_no

-- 1. Add new columns
ALTER TABLE yarn_receipt_items
  ADD COLUMN IF NOT EXISTS cones_per_box SMALLINT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS box_count SMALLINT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS box_no TEXT DEFAULT NULL;

-- 2. Add comments
COMMENT ON COLUMN yarn_receipt_items.cones_per_box IS 'Number of cones per box (e.g. 18 cones)';
COMMENT ON COLUMN yarn_receipt_items.box_count IS 'Total number of boxes in this receipt line';
COMMENT ON COLUMN yarn_receipt_items.box_no IS 'Box number from carton label (e.g. 19)';

-- 3. Recreate RPC: rpc_create_yarn_receipt with packaging fields
...
-- 4. Recreate RPC: rpc_update_yarn_receipt with packaging fields
