DROP FUNCTION IF EXISTS public.rpc_create_bom(jsonb, jsonb);
DROP FUNCTION IF EXISTS public.rpc_create_bom(text, text, uuid);
DROP FUNCTION IF EXISTS public.rpc_create_bom(jsonb, jsonb, uuid);

CREATE OR REPLACE FUNCTION public.rpc_create_bom(
  p_header jsonb,
  p_items jsonb,
...
BEGIN
  ...
  SELECT tenant_id INTO v_tenant FROM profiles WHERE id = v_user_id_final;
