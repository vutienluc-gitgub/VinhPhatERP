...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  process.stderr.write('[mcp-server] SUPABASE_URL hoặc SUPABASE_SERVICE_ROLE_KEY chưa cấu hình\n')
  process.exit(1)
}
...
const server = new Server(
  { name: 'vinhphat-mcp', version: '1.0.0' },
  { capabilities: { tools: {} } },
)
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'list_orders',
      description: 'Lấy danh sách đơn hàng. Có thể lọc theo trạng thái, khoảng thời gian. Trả về mã đơn, khách hàng, ngày, tổng tiền, trạng thái.',
      inputSchema: {
        type: 'object',
        properties: {
          status: {
    ...
    {
      name: 'get_order',
      description: 'Lấy chi tiết đầy đủ một đơn hàng: các mục hàng, số tiền đã trả, trạng thái giao hàng.',
      inputSchema: {
        type: 'object',
        required: ['order_number'],
        properties: {
          order_number: { type: 'string', description: 'Mã số đơn hàng (VD: DH-2025-001)' },
        },
      },
    },
    {
      name: 'list_customers',
      description: 'Lấy danh sách khách hàng. Có thể tìm theo tên hoặc lọc theo trạng thái.',
      inputSchema: {
        type: 'object',
        properties: {
          search: { type: 'string', description: 'Tìm theo tên hoặc mã khách hàng' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          search: { type: 'string', description: 'Tìm theo tên hoặc mã nhà cung cấp' },
          limit: { type: 'number', description: 'Số lượng tối đa (mặc định 20)' },
        },
      },
    },
    {
      ...
      description: 'Lấy danh sách phiếu nhập sợi. Có thể lọc theo nhà cung cấp, trạng thái, khoảng thời gian.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Tổng hợp kinh doanh: tổng đơn hàng, doanh thu, số khách hàng, tình trạng công nợ theo khoảng thời gian.',
      inputSchema: {
        type: 'object',
        properties: {
          from_date: { type: 'string', description: 'YYYY-MM-DD' },
  ...
  try {
    switch (name) {
      ...
      case 'list_orders': {
        ...
        if (args?.from_date) query = query.gte('order_date', args.from_date as string)
        if (args?.to_date) query = query.lte('order_date', args.to_date as string)
      ...
      case 'list_customers': {
        let query = supabase
          .from('customers')
          .select('id, code, name, phone, email, address, status, source, created_at')
          .order('name')
          .limit((args?.limit as number) ?? 20)
      ...
      case 'list_yarn_receipts': {
        ...
        if (args?.status) query = query.eq('status', args.status as string)
        if (args?.from_date) query = query.gte('receipt_date', args.from_date as string)
        if (args?.to_date) query = query.lte('receipt_date', args.to_date as string)
      ...
      case 'get_business_summary': {
        const fromDate = (args?.from_date as string) ?? new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10)
        ...
        const summary = {
          ...
          orders: { total: orders.length, by_status: statusCount },
          revenue: {
