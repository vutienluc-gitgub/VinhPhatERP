...
async function run() {
  const { data: bomYarns } = await supabase
    .from('bom_yarn_items')
    .select(
      '*, bom_templates!inner(code, standard_loss_pct), yarn_catalogs(code, name)',
    )
  ...
  for (const p of prices) {
    if (!priceMap[p.yarn_catalog_id])
      priceMap[p.yarn_catalog_id] = p.unit_price;
  }

  console.log('Prices Map:', priceMap);

  console.log('Deduplicated BOM Yarns Calculation:');
  const uniqueYarns = {};
  ...
  let totalCost = 0;
  const standardLoss = 1; // From the screenshot, standard_loss_pct = 1.00
  const targetQuantity = 1; // Assuming 1 unit
  const targetWeightKg = targetQuantity; // 1 kg
  ...
  for (const key of Object.keys(uniqueYarns)) {
    ...
    console.log(
      `- ${key}: ratio=${req.ratio_pct}%, required_kg=${requiredKg}, price=${price}, item_cost=${itemCost}`,
    );
  }
