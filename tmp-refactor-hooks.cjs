...
function processFile(file, hookModule) {
  ...
  if(content.includes('useActiveCustomers') && !content.includes('@/shared/hooks/useActiveCustomers')) {
...
processFile(path.join(__dirname, 'src', 'features', 'orders', 'OrderForm.tsx'), './useOrders');
processFile(path.join(__dirname, 'src', 'features', 'quotations', 'QuotationForm.tsx'), './useQuotations');
