...
async function checkStandalone() {
  ...
  const { data: allProgress, error: err1 } = await supabase
    .from('order_progress')
    .select('id, order_id, work_order_id')
    .limit(10);
  if (err1) {
  ...
  if (err2) {
    ...
    console.log(
      'Found standalone progress records:',
      standaloneProgress.length,
    );
    if (standaloneProgress.length > 0) {
      console.log(
        'Sample standalone:',
        JSON.stringify(standaloneProgress[0], null, 2),
      );
    }
  }
  ...
  if (err3) {
    ...
    console.log(
      'Found standalone Work Orders in database:',
      standaloneWOs.length,
    );
