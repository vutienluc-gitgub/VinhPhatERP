...
async function createReservedWorkOrder() {
  ...
  const { data: customer, error: custErr } = await supabase.from('customers').select('id').limit(1).single()
  ...
  const { data: bom, error: bomErr } = await supabase.from('bom_templates').select('id, active_version').limit(1).single()
