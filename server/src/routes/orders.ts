...
router.get('/', requireAuth, async (c) => {
  ...
  if (status)     query = query.where(eq(orders.status, status as typeof orders.status._.data))
  if (customerId) query = query.where(eq(orders.customerId, customerId))
  const rows = await query.orderBy(desc(orders.orderDate)).limit(Number(limit)).offset(offset)
...
router.get('/:id', requireAuth, async (c) => {
  ...
  const progress = await db.select().from(orderProgress).where(eq(orderProgress.orderId, id))
  return c.json({ data: { ...order, items, progress } })
})
...
router.post('/', requireAuth, zValidator('json', orderSchema), async (c) => {
  ...
  const result = await db.transaction(async (tx) => {
    const [newOrder] = await tx
      .insert(orders)
      .values({ ...orderData, createdBy: user.id })
      .returning()
