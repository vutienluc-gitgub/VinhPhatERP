...
describe('orderItemSchema', () => {
  ...
  it('trims fabricType whitespace', () => {
    const result = orderItemSchema.parse({ ...validItem, fabricType: '  Cotton  ' })
    expect(result.fabricType).toBe('Cotton')
  })

  it('rejects fabricType shorter than 2 chars', () => {
    const result = orderItemSchema.safeParse({ ...validItem, fabricType: 'A' })
  ...
  it('rejects negative quantity', () => {
    const result = orderItemSchema.safeParse({ ...validItem, quantity: -5 })
    expect(result.success).toBe(false)
  })

  it('accepts zero unitPrice', () => {
    const result = orderItemSchema.parse({ ...validItem, unitPrice: 0 })
    expect(result.unitPrice).toBe(0)
  })

  it('rejects negative unitPrice', () => {
    const result = orderItemSchema.safeParse({ ...validItem, unitPrice: -1 })
    expect(result.success).toBe(false)
  })
...
describe('ordersSchema', () => {
  ...
  it('trims orderNumber whitespace', () => {
    const result = ordersSchema.parse({ ...validOrder, orderNumber: '  DH2603-0001  ' })
    expect(result.orderNumber).toBe('DH2603-0001')
  })

  it('rejects orderNumber shorter than 3 chars', () => {
    const result = ordersSchema.safeParse({ ...validOrder, orderNumber: 'AB' })
    expect(result.success).toBe(false)
  })

  it('rejects invalid customerId (not UUID)', () => {
    const result = ordersSchema.safeParse({ ...validOrder, customerId: 'not-a-uuid' })
    expect(result.success).toBe(false)
  })

  it('rejects empty orderDate', () => {
    const result = ordersSchema.safeParse({ ...validOrder, orderDate: '' })
    expect(result.success).toBe(false)
  })

  it('rejects empty items array', () => {
    const result = ordersSchema.safeParse({ ...validOrder, items: [] })
    expect(result.success).toBe(false)
  })

  it('allows deliveryDate to be empty', () => {
    const result = ordersSchema.parse({ ...validOrder, deliveryDate: '' })
