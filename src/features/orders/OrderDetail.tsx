...
import { useCancelOrder, useCompleteOrder, useConfirmOrder, useOrder } from './useOrders'
...
function statusClass(status: OrderStatus): string {
  switch (status) {
    ...
    case 'completed': return 'in_stock'
    case 'cancelled': return 'damaged'
...
export function OrderDetail({
  ...
  if (!order) return <div className="panel-card"><p className="table-empty">Không tìm thấy đơn hàng.</p></div>

  const balanceDue = order.total_amount - order.paid_amount
  const items = order.order_items ?? []

  async function handleConfirm() {
    const ok = await confirm({ message: 'Xác nhận đơn hàng? Hệ thống sẽ tạo 7 công đoạn tiến độ tự động.' })
    if (!ok) return
    confirmMutation.mutate(orderId)
  }

  async function handleCancel() {
    const ok = await confirm({ message: 'Huỷ đơn hàng này?', variant: 'danger' })
    if (!ok) return
    ...
    <div className="panel-card card-flush">
      ...
      <div style={{ padding: '1.25rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
          ...
          <div>
            ...
            <div className="numeric-cell" style={{ fontWeight: 600 }}>{formatCurrency(order.total_amount)} đ</div>
          </div>
          <div>
            <div className="td-muted summary-label">Đã thu</div>
            <div className="numeric-paid">{formatCurrency(order.paid_amount)} đ</div>
          </div>
          <div>
        ...
        {order.total_amount > 0 && (
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
              ...
              <span>{Math.min(100, Math.round((order.paid_amount / order.total_amount) * 100))}%</span>
            </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
          {order.status === 'draft' && (
            <>
              <button className="btn-secondary" type="button" onClick={() => onEdit(order)}>
                ✏️ Sửa đơn
              </button>
              <button
          ...
          {(order.status === 'confirmed' || order.status === 'in_progress') && (
            <>
              ...
              <button className="btn-secondary" type="button" onClick={() => onCreatePayment(order)}>
                💰 Thu tiền
              </button>
            </>
          )}
        ...
        {(confirmMutation.error || cancelMutation.error || completeMutation.error) && (
          <p style={{ color: '#c0392b', fontSize: '0.88rem' }}>
            Lỗi: {((confirmMutation.error || cancelMutation.error || completeMutation.error) as Error).message}
          </p>
        )}
      </div>
      ...
      {progressStages.length > 0 && (
        <div style={{ padding: '0 1.25rem 1.25rem' }}>
          ...
          <ProgressTimeline stages={progressStages} readonly={order.status === 'cancelled' || order.status === 'completed'} />
        </div>
      )}
    </div>
  )
