...
/** Schema cho 1 dòng cuộn trong bảng nhập nhanh */
...
export function findDuplicateRollNumbers(
  ...
  for (const roll of rolls) {
    const normalizedRollNumber = roll.roll_number.trim()
    if (!normalizedRollNumber) {
      continue
    }

    counts.set(normalizedRollNumber, (counts.get(normalizedRollNumber) ?? 0) + 1)
  }
...
/** Schema cho toàn bộ form nhập nhanh (thông tin chung + danh sách cuộn) */
export const bulkInputSchema = z.object({
  ...
  for (const duplicateRollNumber of duplicateRollNumbers) {
    values.rolls.forEach((roll, index) => {
      if (roll.roll_number.trim() !== duplicateRollNumber) {
        return
      }
      ...
      ctx.addIssue({
        ...
        message: `Mã cuộn "${duplicateRollNumber}" bị trùng trong lô nhập`,
      })
    })
  }
})
