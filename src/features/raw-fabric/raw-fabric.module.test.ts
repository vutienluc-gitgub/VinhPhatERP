...
describe('raw-fabric.module', () => {
  ...
  it('rejects duplicate roll numbers inside bulk input', () => {
    const result = bulkInputSchema.safeParse({
      ...
      fabric_type: 'Dệt thoi 60/40 TC',
      rolls: [
        { roll_number: 'RM-001', weight_kg: 25.5, notes: '' },
        { roll_number: ' RM-001 ', weight_kg: 26.1, notes: '' },
      ],
    })

    expect(result.success).toBe(false)
    if (result.success) {
      throw new Error('Expected bulkInputSchema to reject duplicate roll numbers')
    }

    expect(result.error.issues.some((issue) => issue.path.join('.') === 'rolls.0.roll_number')).toBe(true)
    expect(result.error.issues.some((issue) => issue.path.join('.') === 'rolls.1.roll_number')).toBe(true)
  })
