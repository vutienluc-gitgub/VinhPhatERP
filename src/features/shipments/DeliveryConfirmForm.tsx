...
export function DeliveryConfirmForm({ shipment, onClose }: Props) {
  ...
  return (
    <AdaptiveSheet open={true} onClose={onClose} title={`Xác nhận giao hàng — ${shipment.shipment_number}`} maxWidth={500}>
      <form id="delivery-confirm-form" onSubmit={handleSubmit(onSubmit)}>
        {/* Receiver info */}
        <div className="form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
          <div className="form-field">
            <label className="form-label" htmlFor="receiverName">Tên người nhận <span className="field-required">*</span></label>
            <input id="receiverName" className={`field-input${errors.receiverName ? ' is-error' : ''}`} {...register('receiverName')} placeholder="Họ tên người nhận hàng" />
            {errors.receiverName && <span className="field-error">{errors.receiverName.message}</span>}
          </div>
          <div className="form-field">
            <label className="form-label" htmlFor="receiverPhone">SĐT người nhận</label>
            <input id="receiverPhone" className="field-input" {...register('receiverPhone')} placeholder="0901..." />
          </div>
        </div>

        {/* Photo proof */}
        <div className="form-field">
          <label className="form-label">Ảnh biên nhận / chữ ký <span className="field-required">*</span></label>
          <input
          ...
          />
          {errors.deliveryProof && <span className="field-error">{errors.deliveryProof.message}</span>}
        ...
        <div className="form-field">
          ...
          <textarea id="notes" className="field-textarea" rows={2} {...register('notes')} placeholder="Ghi chú thêm..." />
        </div>
        ...
        <div className="modal-footer" style={{ marginTop: '1.5rem', padding: 0, border: 'none' }}>
          <button className="btn-secondary" type="button" onClick={onClose} disabled={isSubmitting || deliverMutation.isPending}>
            Huỷ
          </button>
          <button
