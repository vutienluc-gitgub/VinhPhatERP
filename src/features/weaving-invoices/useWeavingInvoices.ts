...
export function useWeavingInvoiceList(filters: WeavingInvoiceFilter = {}, page = 1) {
  return useQuery({
...
export function useCreateWeavingInvoice() {
  ...
  return useMutation({
    mutationFn: (values: WeavingInvoiceFormValues) => createWeavingInvoice(values),
    onSuccess: () => {
...
export function useUpdateWeavingInvoice() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, values }: { id: string; values: WeavingInvoiceFormValues }) =>
