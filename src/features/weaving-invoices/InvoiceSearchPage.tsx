...
const searchSchema = z.object({
  ...
  captchaAnswer: z.string().trim().min(1, 'Vui lòng điền kết quả phép tính'),
});
...
export function InvoiceSearchPage() {
  ...
  const onSubmit = (values: SearchFormValues) => {
    ...
    const userAnswer = parseInt(values.captchaAnswer, 10);
    ...
    navigate(`/tra-cuu/${values.lookupCode.trim().toUpperCase()}`);
  };
        ...
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-[#dce6f0] overflow-hidden transition-all duration-300 hover:shadow-2xl">
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-[#0f3460] uppercase tracking-wider">
                Mã tra cứu hóa đơn
              </label>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-3">
                ...
                <input
                  type="number"
                  placeholder="?"
                  className={`flex-1 px-4 py-3 rounded-xl border bg-slate-50 text-[var(--text-primary)] font-bold text-lg text-center focus:outline-none focus:ring-2 focus:ring-[#1a6bb5] focus:bg-white transition-all ${
            ...
            <button
              ...
              className="w-full bg-[#0f3460] hover:bg-[#15467e] text-white font-bold py-3.5 px-4 rounded-xl shadow-lg hover:shadow-xl disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer border-none"
            >
              {isSubmitting ? (
                <>
