    ...
    <div className="panel-card card-flush">
      ...
      <div style={{ padding: '0 1.25rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {orders.map((order) => {
          ...
          const totalCount = order.stages.filter((s) => s.status !== 'skipped').length
          const pct = totalCount > 0 ? Math.round((doneCount / totalCount) * 100) : 0
          const daysOverdue = order.deliveryDate ? getDaysOverdue(order.deliveryDate) : 0
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.4rem' }}>
                <div>
                  ...
                  <span className="td-muted" style={{ marginLeft: '0.5rem' }}>{order.customerName}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.82rem' }}>
                  <span style={{ fontVariantNumeric: 'tabular-nums' }}>{pct}%</span>
                  {order.deliveryDate && (
                    <span style={{ color: daysOverdue > 0 ? '#c0392b' : 'var(--muted)' }}>
                      {order.deliveryDate}
                ...
                {order.stages.map((row) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (!clickable) return
                        const nextStatus = row.status === 'pending' ? 'in_progress' : 'done'
