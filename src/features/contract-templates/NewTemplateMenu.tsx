    ...
    <div className="relative" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2.5 z-[50] w-64 p-2 rounded-2xl border border-border/60 bg-surface shadow-2xl animate-in zoom-in-95 duration-200 origin-top-right">
          <button
            type="button"
            className="w-full text-left px-4 py-3.5 rounded-xl text-sm font-bold hover:bg-blue-600 hover:text-surface flex items-center gap-3 transition-all group"
          ...
          <button
            type="button"
            className="w-full text-left px-4 py-3.5 rounded-xl text-sm font-bold hover:bg-violet-600 hover:text-surface flex items-center gap-3 mt-1 transition-all group"
            onClick={() => handleSelect('purchase')}
          >
            <div className="p-2 rounded-lg bg-violet-500/10 text-violet-600 group-hover:bg-surface/20 group-hover:text-surface transition-colors">
              <Icon name="FileText" size={16} />
            </div>
            Bản mẫu Mua hàng
          </button>
