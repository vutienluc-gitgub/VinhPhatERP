...
export function deleteBlockReason(status: RollStatus): string | null {
  ...
  if (status === 'damaged') return 'Cuộn hư hỏng — không thể xóa, hãy giữ để kiểm tra'
 ...
 */
export function getAllowedStatusTransitions(currentStatus: RollStatus): RollStatus[] {
