      ...
      {rows.length === 0 ? (
        ...
        <table className="data-table">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr
                key={`${row.fabric_type}-${row.color_name}-${row.quality_grade}-${i}`}
              >
                <td>
                  <strong>{row.fabric_type}</strong>
                </td>
                ...
                <td className="numeric-cell">
                  {formatNumber(row.total_length_m)}
                </td>
          ...
          <tfoot>
            <tr>
              ...
              <td className="numeric-cell">
                <strong>
                  {formatNumber(rows.reduce((s, r) => s + r.total_length_m, 0))}
                </strong>
              </td>
              <td className="numeric-cell hide-mobile">
                <strong>
                  {formatNumber(
                    rows.reduce((s, r) => s + r.total_weight_kg, 0),
                  )}
                </strong>
              </td>
            </tr>
