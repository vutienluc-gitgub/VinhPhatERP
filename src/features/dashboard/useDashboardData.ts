...
export function useDashboardStats() {
  return useQuery({
    ...
    queryFn: async (): Promise<DashboardStats> => {
      ...
      const expiringList = qList.filter(q => q.status !== 'converted' && q.status !== 'rejected' && q.valid_until && q.valid_until >= today && q.valid_until <= new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10))
...
export function usePendingTasks(stats: DashboardStats | undefined) {
  if (!stats) return []
...
export function useCustomerSources() {
  return useQuery({
    ...
    queryFn: async (): Promise<CustomerSourceItem[]> => {
      const { data } = await supabase
        .from('customers')
        .select('source')
