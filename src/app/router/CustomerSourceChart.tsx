...
export function CustomerSourceChart({ sources, isLoading }: CustomerSourceChartProps) {
  ...
  return (
    <div className="panel-card" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ padding: '1.25rem 1.25rem 0' }}>
        ...
        <p className="td-muted" style={{ fontSize: '0.85rem', marginTop: '0.35rem' }}>
      ...
      <div style={{ padding: '1rem 1.25rem 1.25rem' }}>
        {isLoading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="skeleton-block" style={{ height: '22px' }} />
            ))}
          </div>
        ) : sources.length === 0 ? (
