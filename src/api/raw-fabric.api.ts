...
export async function createRawFabricBulk(rows: Omit<RawFabricRoll, 'id' | 'created_at' | 'updated_at'>[]): Promise<RawFabricRoll[]> {
  ...
  if ((existing ?? []).length > 0) {
    const taken = existing.map((r) => r.roll_number).sort((a, b) => a.localeCompare(b))
