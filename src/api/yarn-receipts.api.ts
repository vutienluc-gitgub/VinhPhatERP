...
export async function fetchYarnReceiptsPaginated(
  ...
  if (filters.search?.trim()) query = query.ilike('receipt_number', `%${filters.search.trim()}%`)
...
export async function fetchYarnCatalogOptionsForReceipt(): Promise<YarnCatalogOption[]> {
  const { data, error } = await supabase
    .from('yarn_catalogs')
...
export async function createYarnReceiptFull(input: YarnReceiptCreateInput): Promise<YarnReceipt> {
  ...
  await supabase.from(HEADER_TABLE).update({ total_amount: total }).eq('id', headerId)
...
export async function updateYarnReceiptFull(
  ...
  const { error: delErr } = await supabase.from(ITEMS_TABLE).delete().eq('receipt_id', id)
