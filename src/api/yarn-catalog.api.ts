...
export async function fetchYarnCatalogOptions(): Promise<Pick<YarnCatalog, 'id' | 'code' | 'name' | 'composition' | 'color_name' | 'tensile_strength' | 'origin' | 'unit'>[]> {
  const { data, error } = await supabase
    ...
    .order('name')
  if (error) throw error
  return (data ?? []) as Pick<YarnCatalog, 'id' | 'code' | 'name' | 'composition' | 'color_name' | 'tensile_strength' | 'origin' | 'unit'>[]
}
...
export async function createYarnCatalog(row: YarnCatalogRow): Promise<YarnCatalog> {
  const { data, error } = await supabase
    .from(TABLE)
    .insert([row])
    .select()
...
export async function updateYarnCatalog(id: string, row: YarnCatalogRow): Promise<YarnCatalog> {
  const { data, error } = await supabase
    .from(TABLE)
    .update(row)
    .eq('id', id)
