...
export async function fetchSuppliers(filters: { status?: string; category?: string; search?: string } = {}): Promise<Supplier[]> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let query = (supabase as any).from(TABLE).select('*').order('name', { ascending: true })
  if (filters.status) query = query.eq('status', filters.status)
  if (filters.category) query = query.eq('category', filters.category)
