...
export async function fetchProductionEfficiency(): Promise<ProductionEfficiencyRow[]> {
  const { data, error } = await supabase
...
export async function fetchPaymentCollection(): Promise<PaymentCollectionRow[]> {
  const { data, error } = await supabase
