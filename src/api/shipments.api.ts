...
export async function fetchShipmentsPaginated(
  ...
  if (filters.deliveryStaffId) query = query.eq('delivery_staff_id', filters.deliveryStaffId)
...
export async function fetchShipmentsByOrder(orderId: string): Promise<Shipment[]> {
  const { data, error } = await supabase
    .from(HEADER_TABLE)
...
export async function createShipmentFull(input: ShipmentCreateInput): Promise<Shipment> {
  ...
  if (selectedRollIds.length > 0) {
    await supabase
      .from('finished_fabric_rolls')
      .update({ status: 'reserved', reserved_for_order_id: input.orderId })
...
export async function confirmShipmentFull(shipmentId: string): Promise<ShipmentDocument> {
...
export async function markShipmentDelivered(shipmentId: string, values: DeliveryConfirmInput): Promise<void> {
  const { error } = await supabase
    .from(HEADER_TABLE)
    .update({
