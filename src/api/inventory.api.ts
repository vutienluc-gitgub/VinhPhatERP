...
export async function fetchRawFabricInventory(): Promise<{ stats: InventoryStats; breakdown: InventoryBreakdownRow[] }> {
  const { data, error } = await supabase
...
export async function fetchFinishedFabricInventory(): Promise<{ stats: InventoryStats; breakdown: InventoryBreakdownRow[] }> {
  const { data, error } = await supabase
...
export async function fetchYarnInventory(): Promise<{ totalReceipts: number; totalAmount: number }> {
  const { data, error } = await supabase
    .from('yarn_receipts')
...
export async function fetchAgingStock(): Promise<AgingRoll[]> {
  ...
  const [{ data: rawData, error: rawErr }, { data: finishedData, error: finishedErr }] =
    await Promise.all([
      supabase
