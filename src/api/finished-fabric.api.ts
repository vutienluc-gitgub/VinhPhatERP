...
export async function fetchRawRollsByLot(
  lotNumber: string,
): Promise<RawRollOption[]> {
  const { data, error } = await supabase
    .from('raw_fabric_rolls')
    .select('id, roll_number, fabric_type, color_name, lot_number')
...
export async function fetchTraceChain(
  rawRollId: string,
): Promise<TraceChainData> {
  const result: TraceChainData = {
    rawRoll: null,
    yarnReceipt: null,
  };
  ...
  const weavingPartner = raw.suppliers as {
    id: string;
    name: string;
    code: string;
  } | null;
  ...
  const { data: receiptData, error: receiptError } = await supabase
    .from('yarn_receipts')
    .select(
      'id, receipt_number, receipt_date, total_amount, status, suppliers(id, name, code), yarn_receipt_items(id)',
    )
    .eq('id', yarnReceiptId)
    .single();
  ...
  const supplier = receipt.suppliers as {
    id: string;
    name: string;
    code: string;
  } | null;
