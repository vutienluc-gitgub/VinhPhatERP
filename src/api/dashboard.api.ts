...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  ...
  const [drafts, active, overdue, debt, payments, shipments, quotations] = await Promise.all([
    ...
    supabase.from('payments').select('amount').gte('payment_date', sevenDaysAgo),
  ...
  const expiringList = qList.filter(
    (q) => q.status !== 'converted' && q.status !== 'rejected' && q.valid_until && q.valid_until >= today && q.valid_until <= in3Days,
  )

  const totalDecided = qList.filter((q) => ['converted', 'rejected', 'expired'].includes(q.status)).length
  const totalConverted = qList.filter((q) => q.status === 'converted').length
  const conversionRate = totalDecided > 0 ? Math.round((totalConverted / totalDecided) * 100) : null
...
export function buildPendingTasks(stats: DashboardStats): PendingTask[] {
  ...
  if (stats.overdueOrders > 0) {
    tasks.push({ icon: '⚠️', text: 'Đơn hàng trễ hạn', count: stats.overdueOrders, href: '/orders', isAlert: true })
  }
  if (stats.expiringQuotations > 0) {
    tasks.push({ icon: '📋', text: 'Báo giá sắp hết hạn', count: stats.expiringQuotations, href: '/quotations', isAlert: true })
  }
  if (stats.pendingShipments > 0) {
    tasks.push({ icon: '📦', text: 'Phiếu xuất chờ xử lý', count: stats.pendingShipments, href: '/shipments', isAlert: false })
  }
  if (stats.draftOrders > 0) {
    tasks.push({ icon: '📝', text: 'Đơn nháp chưa xác nhận', count: stats.draftOrders, href: '/orders', isAlert: false })
  }
  if (stats.totalDebt > 0) {
    tasks.push({ icon: '💰', text: `Công nợ còn ${formatCurrency(stats.totalDebt)} đ`, count: 0, href: '/payments', isAlert: stats.totalDebt > 50_000_000 })
  }
