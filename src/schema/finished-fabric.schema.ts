...
export function findDuplicateRollNumbers(rolls: Array<{ roll_number: string }>): string[] {
  ...
  return Array.from(counts.entries()).filter(([, c]) => c > 1).map(([n]) => n)
}
...
export const finishedFabricDefaults: FinishedFabricFormValues = {
  roll_number: '', raw_roll_id: '' as unknown as string, fabric_type: '',
  color_name: '', color_code: '', width_cm: undefined, length_m: undefined,
  weight_kg: undefined, quality_grade: undefined, status: 'in_stock',
  warehouse_location: '', production_date: '', reserved_for_order_id: null, notes: '',
}
  ...
  for (const dup of duplicates) {
    values.rolls.forEach((roll, i) => {
      if (roll.roll_number.trim() === dup) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['rolls', i, 'roll_number'], message: `Mã cuộn "${dup}" bị trùng trong lô nhập` })
      }
    })
  }
})
...
export const bulkFinishedInputDefaults: BulkFinishedInputFormValues = {
  lot_number: '', fabric_type: '', color_name: '', color_code: '', width_cm: undefined,
  quality_grade: undefined, status: 'in_stock', warehouse_location: '', production_date: '',
  roll_prefix: 'FN-', start_number: 1,
  rolls: [{ roll_number: formatBulkRollNumber('FN-', 1), raw_roll_id: '' as unknown as string, weight_kg: undefined as unknown as number, length_m: undefined, quality_grade: undefined, notes: '' }],
}
