...
## Quy trình tổng quát cho AI Agent (Training Note)

**Quy trình xử lý đơn hàng chuẩn:**

Khách → Báo giá → Tạo đơn → Xác nhận → Xuất kho → Giao → Thu tiền

- Mỗi bước là một trạng thái/bước nghiệp vụ quan trọng, cần thực hiện tuần tự.
- AI agent phải hiểu rõ:
  - Không được bỏ qua bước nào.
  - Mỗi bước có thể có điều kiện kiểm tra hoặc xác nhận (ví dụ: kiểm tra công nợ trước khi tạo đơn, xác nhận tồn kho trước khi xuất kho).
  - Ghi chú, cảnh báo hoặc yêu cầu xác nhận khi gặp trường hợp đặc biệt (nợ quá hạn, thiếu hàng, v.v.).
- Đây là quy trình chuẩn để training AI agent và hướng dẫn nhân sự mới.
